...
/**
 * Explainable rule engine for T-027. It only emits struggle types for which the
 * current Cycle-1 evidence contract has a defensible signal; unsupported types
 * are deliberately left un-inferred rather than guessed.
 */
@Service
public class StruggleInferenceService {
    ...
    public StruggleInferenceService(StruggleInferenceRepository inferences,
    ...
    public void onAssessmentEvidence(AssessmentEvidenceRecordedEvent event) {
        ...
        for (UUID topicId : event.topicNodeIds()) {
            ...
            if (event.selfDoubtFlag()) {
                save(event.learnerId(), topicId, StruggleType.METACOGNITIVE,
                        "5a_self_efficacy", SELF_DOUBT_PROBABILITY,
                        Map.of("signal", "self_doubt_flag", "attempt_id", event.attemptId().toString()),
                        now, expires);
            }
        }
    }
    ...
    private void inferPrerequisiteGap(UUID learnerId, UUID topicId, Instant now, Instant expires) {
        ...
        var weak = prerequisites.stream()
                .map(p -> Map.entry(p, states.get(p.id())))
                .filter(e -> e.getValue() != null && e.getValue().mastery() < PREREQUISITE_THRESHOLD)
                .toList();
        if (weak.isEmpty()) {
            return;
        }
        ...
        save(learnerId, topicId, StruggleType.PREREQUISITE_GAP,
                "1a_weak_prerequisite_mastery", probability, evidence, now, expires);
    }

    private void inferProceduralFluency(UUID learnerId, UUID topicId, Instant now, Instant expires) {
        SkillState state = learnerModel.skillStates(learnerId).stream()
                .filter(s -> topicId.equals(s.nodeId()))
                .findFirst().orElse(null);
        if (state == null || state.proceduralFluencyGap() == null
                || state.proceduralFluencyGap() < FLUENCY_GAP_THRESHOLD) {
            return;
        }
        double gap = state.proceduralFluencyGap();
        double probability = clamp(0.50 + gap, 0.50, 0.95);
        save(learnerId, topicId, StruggleType.EXAM_LITERACY,
                "3b_procedural_fluency", probability,
                Map.of("signal", "procedural_fluency_gap", "gap", gap), now, expires);
    }

    private void save(UUID learnerId, UUID topicId, StruggleType type, String subtype,
                      double probability, Map<String, Object> evidence,
                      Instant generatedAt, Instant expiresAt) {
        inferences.save(new StruggleInference(learnerId, topicId, type, subtype,
                probability, evidence, MODEL_VERSION, generatedAt, expiresAt));
    }
