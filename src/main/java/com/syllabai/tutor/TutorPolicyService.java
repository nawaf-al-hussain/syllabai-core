...
public class TutorPolicyService {
    ...
    public InterventionPlan select(UUID learnerId,
        ...
        events.publishEvent(new TutorInterventionSelectedEvent(learnerId, List.copyOf(topicIds), selected.type().name(), selected.rationale(), POLICY_VERSION, Instant.now()));
        return selected;
    }

    private InterventionPlan planFor(StruggleInference i) {
        return switch (i.type()) {
            case PREREQUISITE_GAP -> plan(InterventionType.PREREQUISITE_REVIEW, i.subtype(), List.of(
                    "Review the weakest prerequisite before the target concept.", "Connect it back to the learner's question.", "Check understanding before moving forward."));
            case EXAM_LITERACY -> plan(InterventionType.PROCEDURAL_FLUENCY, i.subtype(), List.of(
                    "Explain the method briefly.", "Give a small timed-style verification step.", "Encourage transfer to a nearby exam-style task."));
            case METACOGNITIVE -> plan(InterventionType.METACOGNITIVE_CHECK, i.subtype(), List.of(
                    "Use a low-pressure verification step.", "Ask the learner to state why their answer is justified.", "Avoid framing the learner as deficient."));
            default -> plan(InterventionType.EXPLANATION, "unsupported struggle category for v0 intervention rules",
                    List.of("Use a normal source-grounded explanation."));
        };
    }
    private static InterventionPlan plan(InterventionType type, String rationale, List<String> actions) {
        return new InterventionPlan(type, rationale, actions);
    }
