...
class InterventionRunServiceTest {
    ...
    void terminalRunCannotBeChanged() {
        ...
        run.activate(java.time.Instant.now());
        run.complete("EVIDENCE_COLLECTED", java.time.Instant.now());

        assertThrows(IllegalStateException.class, () -> run.activate(java.time.Instant.now()));
        assertThrows(IllegalStateException.class, () -> run.cancel(java.time.Instant.now()));
    }
